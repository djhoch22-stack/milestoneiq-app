-- ────────────────────────────────────────────────────────────────────────────
-- "Longest …" career rollup = MAX (not SUM)
-- ────────────────────────────────────────────────────────────────────────────
-- The "Longest …" stats (Longest Completion, Longest Rush, Longest Reception, and
-- the special-teams ones) are single-PLAY maxes read from the MaxPreps "Lng" column
-- in each category. A player's CAREER longest is the MAX across their seasons, NEVER
-- the sum (three 80-yard runs is still an 80-yard longest, not 240).
--
-- This redefines recompute_career_from_seasons so any stat key named "Longest …"
-- aggregates with max() while everything else still sums, then rebuilds the football
-- careers. It is a no-op for the longest values until you re-upload the PDFs that now
-- carry those columns (deploy the updated extract-pdf function first), but it is safe
-- to run now and for every sport (no sport but football has "Longest …" keys).
--
...
-- Rebuild DC football now so the new max() logic is in effect (longest values appear after you
-- re-upload the PDFs that carry the "Lng" columns). Harmless if the program isn't present.
