...
export default async function handler(req, res) {
  ...
  const script =
    ...
    `function fmtR(f,v){if(v==null||isNaN(v))return "—";if(f==="pct")return v+"%";if(f==="era2")return Number(v).toFixed(2);if(f==="perSet")return Number(v).toFixed(2);if(f==="per1")return Number(v).toFixed(1);var s=Number(v).toFixed(3);return s.charAt(0)==="0"?s.slice(1):s;}` +
