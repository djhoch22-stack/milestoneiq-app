...
export default async function handler(req, res) {
  ...
  const script =
    ...
    `function rv(s,d){var sp=d.spec;if(sp.kind==="pct")return pcv(s,sp.made,sp.att);if(sp.kind==="ratio"){var n=0,m=0,i;for(i=0;i<sp.num.length;i++)n+=sp.num[i][1]*gv(s,sp.num[i][0]);for(i=0;i<sp.den.length;i++)m+=sp.den[i][1]*gv(s,sp.den[i][0]);return m>0?n/m:null;}if(sp.kind==="ops"){var ab=gv(s,"At Bats");if(ab<=0)return null;var od=ab+gv(s,"Walk (BB)")+gv(s,"Hit By Pitch")+gv(s,"Sacrifice Fly");var ob=od>0?(gv(s,"Hits")+gv(s,"Walk (BB)")+gv(s,"Hit By Pitch"))/od:0;var sl=(gv(s,"Hits")+gv(s,"Doubles")+2*gv(s,"Triples")+3*gv(s,"Home Runs"))/ab;return ob+sl;}if(sp.kind==="era"){var ipv=gv(s,"Innings Pitched");var ip=Math.floor(ipv)+Math.round((ipv-Math.floor(ipv))*10)/3;return ip>0?7*gv(s,"Earned Runs")/ip:null;}return null;}` +
