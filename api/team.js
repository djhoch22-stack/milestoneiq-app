...
export default async function handler(req, res) {
  ...
  const rdJson = JSON.stringify(rateDefsFor(team.sport).map((d) => ({ n: d.name, s: d.short, after: d.after, fmt: d.fmt, q: d.qualStat, mc: d.minCareer, ab: d.noteAbbr, spec: d.spec })));
  ...
  const script =
    ...
    `function rv(s,d){var sp=d.spec;if(sp.kind==="pct")return pcv(s,sp.made,sp.att);if(sp.kind==="ratio"){var n=0,m=0,i;for(i=0;i<sp.num.length;i++)n+=sp.num[i][1]*gv(s,sp.num[i][0]);for(i=0;i<sp.den.length;i++)m+=sp.den[i][1]*gv(s,sp.den[i][0]);return m>0?n/m:null;}if(sp.kind==="ops"){var ab=gv(s,"At Bats");if(ab<=0)return null;var od=ab+gv(s,"Walk (BB)")+gv(s,"Hit By Pitch")+gv(s,"Sacrifice Fly");var ob=od>0?(gv(s,"Hits")+gv(s,"Walk (BB)")+gv(s,"Hit By Pitch"))/od:0;var sl=(gv(s,"Hits")+gv(s,"Doubles")+2*gv(s,"Triples")+3*gv(s,"Home Runs"))/ab;return ob+sl;}return null;}` +
    `function fmtR(f,v){if(v==null||isNaN(v))return "—";if(f==="pct")return v+"%";var s=Number(v).toFixed(3);return s.charAt(0)==="0"?s.slice(1):s;}` +
    ...
    `function row(p,i,st,mx,d){var v=d?(rv(p.s,d)||0):(Number(p.s[st])||0),b=Math.round(v/mx*100),bd=(p.st?"⭐ ":"")+(p.sh?"🏛️ ":"");var vs=d?fmtR(d.fmt,v):v.toLocaleString();return '<tr data-p="'+e(p.k)+'"><td class="rank">'+(i+1)+'</td><td><b>'+e(p.n)+"</b> "+bd+(p.a?'<span class="badge active">Active</span>':"")+(p.y?'<div style="font-size:11px;color:#9ca3af">'+e(p.y)+"</div>":"")+'</td><td class="num"><b>'+vs+'</b></td><td><div style="height:6px;background:#f0f0ee;border-radius:3px"><div style="width:'+b+'%;height:100%;background:'+(p.a?"#1a56db":"#94a3b8")+';border-radius:3px"></div></div></td></tr>';}` +
    `function lb(){if(!sel)return;var st=sel.value,d=rdOf(st),t=(q.value||"").toLowerCase();var vof=function(p){if(!d)return Number(p.s[st])||0;if(gv(p.s,d.q)<d.mc)return 0;return rv(p.s,d)||0;};var rows=ATL.filter(function(p){if(f==="current"&&!p.a)return false;if(f==="alumni"&&p.a)return false;if(t&&p.n.toLowerCase().indexOf(t)<0)return false;return vof(p)>0;}).sort(function(x,y){return vof(y)-vof(x);});var mx=rows.length?(vof(rows[0])||1):1;hd.textContent=st;tb.innerHTML=rows.map(function(p,i){return row(p,i,st,mx,d);}).join("");info.textContent=rows.length+" players"+(d?" · min "+d.mc+" "+d.q:"");}` +
